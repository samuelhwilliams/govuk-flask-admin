...
def integration_app():
    ...
    with app.app_context():
        ...
        admin.add_view(GovukModelView(Author, db.session))
        admin.add_view(GovukModelView(Book, db.session))
...
class TestSelectWithSearchIntegration:
    ...
    def test_single_select_relationship_uses_regular_select(self, integration_app):
        ...
        with app.test_client() as client:
            ...
            assert 'multiple' not in html or 'name="author"' not in html.split('multiple')[0].split('name="author"')[-1]
